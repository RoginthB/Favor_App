  ...
  const handleSubmit = (e) => {
    ...
    try {
      const formData = {
        ...
        userId:
          userData.name + userData.dateOfBirth[8] + userData.dateOfBirth[9],
      };

      axios
        //.post("http://localhost:5000/user/newUser", formData)
        .post(process.env.REACT_APP_API_URL + "/user/newUser", formData)
        .then((res) => {
          if(res.status ===200){
            setOpen(false);
            ...
            setOpen(false);
            alert(`${formData.name } this email Id we have already. !!`);
            navigate("/Login");
          }else{
            setOpen(false);
